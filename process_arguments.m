function varargout = process_arguments(userArgs, varargin)
...
%% ----------------------------------------------------------------------------------------------------------------------------
% This function was originally designed by Matthew Dunham and it was called processArgs. I changed the function such that
% it can accept a cell array as the varargin. This approach makes this function easier to use. I also added a functionality such
% that the required "-" symbols (prefixes) will be automatically added. The original method which was
% passing each varargin separately still works. I called the new function process_arguments to distinguish it from the original
% processArgs.
% Farhad Kamangar Dec. 20, 2010
%
%% ---------------------------------------------------------------------------------------------------------------------------
% USAGE:
%
...
if iscell(varargin) && (numel(varargin)==1) && iscell(varargin{1})
    % The varargin is a cell array with one element which is also a cell array. This means that the defaults arguments were
    % passed to this routine by a cell array. This also means that the required prefixes ("-") may not be correctly passed
    varargin=varargin{1};
    for i=1:numel(userArgs)
        arg = lower(userArgs{i});
        if ~ischar(arg), continue; end
        for j = 1:2:numel(varargin)
            if ~isempty(regexp(lower(varargin{j}),['\<[*\+]?',arg,'\>']))
%             if strcmpi(arg, varargin{j});
                userArgs{i} = ['-', arg];
            end
        end
    end
